<?php
/**
 * @version   $Id: roksprocket.php 29014 2015-10-15 14:17:27Z jakub $
...
if (!defined('ROKSPROCKET_VERSION')) define('ROKSPROCKET_VERSION', '2.1.11');
...
if (!preg_match('/project.version/', ROKCOMMON) && version_compare(preg_replace('/-SNAPSHOT/', '', ROKCOMMON), ROKSPROCKET_NEEDED_ROKCOMMON_VERSION, '<')) {
	rs_force_deactivate();
	roksprocket_set_admin_message('error', __('RokSprocket v2.1.11 needs at least RokCommon Version ' . ROKSPROCKET_NEEDED_ROKCOMMON_VERSION . '.  You currently have RokCommon Version ' . ROKCOMMON));
	return;
}
...
if (!function_exists('roksprocket_mootools_init')) {
	...
	function roksprocket_mootools_init()
	{
		...
		if ((is_admin() && $pagenow == 'widgets.php' && !defined('GANTRY_VERSION')) || (is_admin() && $pagenow == 'admin-ajax.php') || (is_admin() && $page == 'roksprocket-list') || (is_admin() && $page == 'roksprocket-edit') || (is_admin() && ($pagenow == 'post.php' || $pagenow == 'post-new.php')) || (is_admin() && $pagenow == 'media-upload.php') || is_customize_preview()) {
